...
function computeStats(windowId, now = Date.now()) {
  ...
  const entries = typeof windowId === 'number'
    ? switchHistory.filter((entry) => entry.windowId === windowId)
    : switchHistory.slice();
...
function getTabStats(windowId) {
  const stats = computeStats(windowId, Date.now());
  return stats;
}
